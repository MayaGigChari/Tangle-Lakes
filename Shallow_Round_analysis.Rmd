...
#there are a few ways we can deal with the fish that moved

#1. we consider the two fish that moved lakes as both capped and recapped in shallow-round
#2. We consider the two fish that moved as both capped and recpaped in lower
#3. we consider the two fish that moved as capped in lower and recapped in shallow-round and explore some darroch-type estimator. 
#4. we remove the fish that moved from the sample. 
...
#deal with the fish that moved

movers<- data_full_shallowround_truncated_pop%>%
  filter(Floy %in% c("19970", "18608"))


#this is a hacky way, but need to add all these to n1 so they are properly counted.
#other fields don't matter. 
movers_cap <- movers
movers_cap$cap_label<- "n1"
movers_cap$stat<- "n1"
movers_cap$grouping<- "n1"

movers<- rbind(movers, movers_cap)

#1: remove the fish that moved. 
shallow_round_lake_data_nomovers<- data_full_shallowround_truncated_pop%>%
  filter(!Floy %in% c("19970", "18608"))

#2: add the fish that moved

shallow_round_lake_data_withmovers<- rbind(shallow_round_lake_data_nomovers, movers)
...
petersen_estimate_trunc<- pop_calc(petersen_funcs,  shallow_round_lake_data_withmovers)
Chapman_estimate_trunc<- pop_calc(Chapman_funcs, shallow_round_lake_data_withmovers)
bailey_estimate_trunc<- pop_calc(Bailey_funcs, shallow_round_lake_data_withmovers)
...
#data without considering the two floys that moved 
Lower_lake_data_nomovers<- data_full_Lower_truncated_pop%>%
  filter(!Floy %in% c("19970", "18608"))


movers<- data_full_Lower_truncated_pop%>%
  filter(Floy %in% c("19970", "18608"))


#data with considering the two floys that moved as having stayed in lower. 

#this is a hacky way, but need to add all these to n1 so they are properly counted.
#other fields don't matter. 
movers_cap <- movers
movers_cap$cap_label<- "m2"
movers_cap$stat<- "m2"
movers_cap$grouping<- "m2"
...
Lower_lake_data_withmovers<- rbind(Lower_lake_data_nomovers, movers)



#now we do the pop estimates. First with the two floys included. 

petersen_estimate_trunc_Lower<- pop_calc(petersen_funcs,  Lower_lake_data_withmovers)
Chapman_estimate_trunc_Lower<- pop_calc(Chapman_funcs, Lower_lake_data_withmovers)
bailey_estimate_trunc_Lower<- pop_calc(Bailey_funcs, Lower_lake_data_withmovers)
...
#### 3.2.2 Petersen estimation of population size
```{r, echo = FALSE}
